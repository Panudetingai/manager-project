...
type IdFromChat<F extends (...args: never[]) => unknown> = Extract<
  Parameters<F>[0],
  string
>;

export type GoogleModelId = IdFromChat<GoogleGenerativeAIProvider["chat"]>;
export type AnthropicModelId = IdFromChat<
  ReturnType<typeof createAnthropic>["chat"]
>;
...
export type OpenrouterModelId = IdFromChat<
...
export interface OptionParameter {
  id: string;
  message_id: string;
  option: Parameters<typeof streamText | typeof generateText>[0];
  messages: UIMessage[];
}
